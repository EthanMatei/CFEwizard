# TEST SCRIPT - NOT USED IN CFE WIZARD
...
#--------------------------------------------------
timingColumns <- c("task","time (minutes)","description") 
timing <- data.frame(matrix(nrow = 0, ncol = length(timingColumns)), stringsAsFactors = TRUE) 
...
#----------------------------------------
# Set end pre-processing time
#----------------------------------------
endPrePreprocessingTime <- Sys.time()
...
for ( uniqueSubject in list.of.unique.subjects  ) { # loop through individual subject IDs
  ...
  # TIMING
  endSection1Time <- Sys.time()
  section1Time <- section1Time + difftime(endSection1Time, startSection1Time, units="mins")
  
  # TIMING
  startSection2Time <- Sys.time()
  ...
  # TIMING
  endSection2Time <- Sys.time()
  section2Time <- section2Time + difftime(endSection2Time, startSection2Time, units="mins")
  
  # TIMING
  startSection3Time <- Sys.time()
  if (totalVisits > 2){
  ...
  # TIMING
  endSection3Time <- Sys.time()
  section3Time <- section3Time + difftime(endSection3Time, startSection3Time, units="mins")
  
}

startPostProcessingTime <- Sys.time()
...
time <- difftime(endPrePreprocessingTime, startTime, unit="mins")
timing[nrow(timing) + 1,] = c("pre-processing", time, "Time for all processing before the start of the main loop")

timing[nrow(timing) + 1,] = c("section 1", section1Time, "Time for section 1 of main loop")

timing[nrow(timing) + 1,] = c("section 2", section2Time, "Time for section 2 of main loop")

timing[nrow(timing) + 1,] = c("section 3", section3Time, "Time for section 3 of main loop")

time <- difftime(endTime, startPostProcessingTime, unit="mins")
timing[nrow(timing) + 1,] = c("post-processing", time, "Time for all processing after end of the main loop")

time <- difftime(endTime, startTime, unit="mins")
timing[nrow(timing) + 1,] = c("total", time, "Total time")

timingFile <- paste(tempDir, "/discovery-timing_", PHENE, "_", dxChoice, "_",Sys.Date(),".csv", sep="")
write.csv(timing, timingFile)
cat("Timing file created: ", timingFile, "\n")
